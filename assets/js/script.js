...
function getCityData(city) {
  ...
  .then(data => {
    oneCallApi = `https://api.openweathermap.org/data/2.5/onecall?lat=${data.coord.lat}&lon=${data.coord.lon}&appid=${appId}`;
...
function currentWeather(city, data) {
  ...
  currentWeatherInfo.classList.add('font-weight-bold');
